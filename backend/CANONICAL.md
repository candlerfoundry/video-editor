...
## Frame-picker freeze diagnostic — REMOVED (added June 24, removed June 26, 2026)

A passive always-on watchdog + breadcrumb capture (localStorage) was temporarily
added to index.html to catch the intermittent frame-picker freeze. It did its job:
the freeze was root-caused to a SECOND concurrent `<video>` decode in the picker
and fixed — see "Frame-picker freeze fix: release concurrent video decoders". The
instrumentation (the `window._fveBC` breadcrumbs, the 250ms watchdog interval, the
`[FVE FREEZE DIAGNOSTIC]` console report, and all `_fveBC(...)` call sites) was
REMOVED June 26, 2026 now that the cause is fixed. Do not re-add unless a new
intermittent freeze needs capturing.
...
## 2026-06-30 — Cover-burn audio static (44.1kHz resample) — build `2026-06-30-coveraudio`
**Symptom:** Abby's (Unstuck) clips had a consistent medium-pitch fuzz/static, audible on local playback (QuickTime), Dropbox, and Instagram. Jacqui's (TheoEd) clips and another Unstuck clip were clean. Abby clips exported WITHOUT a burned thumbnail were clean; WITH the thumbnail burned they had static — at any speed (1x and sped).
**Root cause:** The cover-burn step (prepending the thumbnail as a ~1/30s frame for the IG reel cover) re-encoded the clip audio and forced `aformat=sample_rates=48000`. Abby's source is **44.1 kHz**, so this triggered a 44.1→48 kHz resample; Jacqui's source is already 48 kHz (no resample). On the user's ffmpeg build that resample introduced audible static. (Could NOT be reproduced in the sandbox ffmpeg — strongly implicates the user's local ffmpeg's default resampler.)
**Fix:** New module-level helper `_burn_cover_frame(thumb_local_path, src_video_path, out_path)` replaces both inline cover-burn blocks (in `export_clip` and `export_thumbnail`). It **probes the clip's native audio sample rate** (ffprobe derived from FFMPEG_EXE), generates the 0.1s lead-silence at that SAME rate, and concats with `aformat=sample_rates=<native>` on both segments — i.e. **NO sample-rate conversion** of the real audio. Also bumps the cover-burn AAC to `-b:a 256k`. If the probe fails it falls back to 48000 (old behavior). Verified: burning a 44.1 kHz clip now yields a 44.1 kHz output (no resample), burn succeeds, +0.108s cover frame.
**Note:** This is a BACKEND change — requires zip rebuild + launcher restart. Loose `server.py` and `foundry-video-editor-backend.zip` both updated. Frontend `EXPECTED_BACKEND_BUILD` bumped to match (handshake only; no other frontend change).

## 2026-06-30 — HOTFIX `coveraudio2`: cover-burn helper stole the /export_clip route
The `coveraudio` insertion placed `def _burn_cover_frame(...)` BETWEEN `@app.route("/export_clip", methods=["POST"])` and `def export_clip():`, so the decorator bound to the helper instead of the handler. Flask then called `_burn_cover_frame()` with no args on every POST /export_clip → `TypeError: missing 3 required positional arguments` → 500 (frontend saw an HTML error page: "Unexpected token '<'"). The file looked correct line-by-line (calls had args); only the decorator placement was wrong. Fix: moved the helper to a plain top-level def ABOVE the `# ── Export Clip ──` banner so the route decorator again attaches to `export_clip`. LESSON: never insert a function between a decorator and its def; insert helpers before the decorator/banner.
...
## 2026-06-30 — Thumbnail editor overlay interaction fixes (drag/resize erratic behavior)
Symptom: with several overlapping text boxes + graphics, dragging/resizing/font changes did "random" things and the source frame sometimes vanished. Root causes (in index.html dialog overlay layer), NOT just z-order:
1. `renderDialogOverlayLayer()` (which does `layer.innerHTML=''`) was called on EVERY mousemove during a drag — destroying+recreating the node being dragged ~60x/s. Fix: new `_dlgLiveDragNode(d)` mutates only the active node's left/top/width/height/fontSize during the drag; full re-render (+ syncDraftFromDialogState) happens once on mouseup.
2. Two mousedown handlers fired per graphic — a per-element `wrap.addEventListener('mousedown', …)` in renderDialogOverlayLayer AND the global document handler. Removed the per-element one; the document handler already selects + drags (via dlgSelectDesignElement, which renders the elem list).
3. The background PAN handler shared the same document mousedown and grabbed the frame on any press not cleanly on an element — sliding it off-screen ("lost image"). Fix: pan now requires the hand tool or Shift (`if (!_dlgPanTool && !e.shiftKey) return;`); a plain drag never pans.
